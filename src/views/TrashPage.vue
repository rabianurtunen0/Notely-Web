<template>
  <div>
    ...
    <div
      ...
      <div v-else id="main" ref="main" class="justify-center place-content-center">
        <div class="flex justify-center mt-16">
          <img
            src="../assets/images/trash_picture.svg"
            class="max-w-xl w-5/6 h-5/6"
          />
        </div>
        <div class="justify-center items-center mx-12">
          <h3 class="font-serif mt-6 text-[#2a2b2e]">
            You don't have deleted note or list.
          </h3>
        </div>
      </div>
    </div>
...
const getNotes = async () => {
  ...
  const querySnapshot = await getDocs(
    collection(doc(db, "Users", userEmail), "Trash")
  );
  console.log(querySnapshot);
  querySnapshot.forEach((doc) => {
    trashList.value.push({
  ...
  console.log("Silinen notların listesi", trashList.value);
};
</script>
